...
#ifdef MANDELBROT_SCALAR
#include <array>
std::array<double,2> operator+(const std::array<double, 2> x, const std::array<double, 2> y) {
...
namespace {
    ...
    Vec vec_init(double value)       { return {value, value}; }
    bool vec_is_any_le(Vec v, Vec f) { return v[0] <= f[0] || v[1] <= f[1]; }
    int vec_is_le(Vec v1, Vec v2)    { return ((v1[1] <= v2[1]) << 1) | (v1[0] <= v2[0]); }
    const uint8_t k_bit_rev[] = { 0b00, 0b10, 0b01, 0b11 };
    void pv(int i, Vec v) {
      printf("%d %f %f\n", i, v[0], v[1]);
    }
#else
    typedef __f64x2 Vec;
    Vec vec_init(double value)       { return wasm_f64x2_splat(value); }
    bool vec_is_any_le(Vec v, Vec f) { return wasm_i64x2_any_true(wasm_f64x2_le(v, f)); }
    int vec_is_le(Vec v1, Vec v2)    { return ((v1[1] <= v2[1]) << 1) | (v1[0] <= v2[0]); }
    const uint8_t k_bit_rev[] = { 0b00, 0b10, 0b01, 0b11 };
    void pv(int i, Vec v) {
      printf("%d %f %f\n", i, v[0], v[1]);
...
#ifdef GEN_IMAGE
    // because of the way fwrite works in emscripten (convert string to utf-8)
    // writing raw bytes to stdout doesn't work, so there is a hack here to
    // convert the pixels into hex codes (strings) and write them out, then in
    // the Makefile there is a postprocess.py that converts the hex back into
    // bytes, reconstructing the mandelbrot set image.
    printf("P4\n%d %d\n", width, height);
